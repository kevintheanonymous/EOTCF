...
const Layout = ({ children }) => {
  ...
  const navItems = [
    ...
    { path: '/users', label: t('users'), icon: "M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z", roles: ['admin'] }, // <--- 3. ADDED COMMA HERE
    { path: '/profile', label: t('profile') || 'Profile', icon: "M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z", roles: ['admin', 'treasurer', 'member'] }
  ]
  ...
  return (
    <div className="min-h-screen bg-warm-parchment flex flex-col lg:flex-row">
      ...
      {isMobileMenuOpen && <div className="fixed inset-0 bg-black/50 z-30 lg:hidden backdrop-blur-sm" onClick={() => setIsMobileMenuOpen(false)} />}

      <div className={`fixed inset-y-0 left-0 w-72 bg-white border-r border-stone-200 z-40 flex flex-col shadow-2xl lg:shadow-none transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:static lg:h-screen lg:inset-auto`}>
        <div className="p-8 pb-4 hidden lg:block">
          <div className="flex items-center gap-3 text-deep-gold">
            <div className="p-2 bg-amber-50 rounded-lg"><EOTCCross /></div>
        ...
        <nav className="flex-1 px-4 space-y-2 mt-4 lg:mt-4 pt-4 lg:pt-0">
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.path} to={item.path} onClick={() => setIsMobileMenuOpen(false)} className={`flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all duration-200 group ${isActive ? 'bg-gold-gradient text-white shadow-glow' : 'text-stone-500 hover:bg-stone-50 hover:text-deep-gold'}`}>
